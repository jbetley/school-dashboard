...
def update_academic_information_page(data, school, year, radio_value):
    ...
    if radio_value == 'proficiency':
        ...
        k8_academic_achievement_data = pd.read_csv(growth_file, skiprows=33, nrows=3)
        hs_academic_achievement_data = pd.read_csv(growth_file, skiprows=37, nrows=3)
        k8_academic_progress_data = pd.read_csv(growth_file, skiprows=41, nrows=3)
        hs_academic_progress_data = pd.read_csv(growth_file, skiprows=45, nrows=3)
        closing_achievement_gap_data = pd.read_csv(growth_file, skiprows=49, nrows=3)
        ...
        ## TODO: Style and align tables
        ## TODO: Do not display is information does not exist

        if not k8_overall_indicators_data.isnull().all().all():
            k8_overall_indicators = create_academic_info_table(k8_overall_indicators_data,'TST')
        else:
            k8_overall_indicators = no_data_table('TST')

        if not hs_overall_indicators_data.isnull().all().all():
            hs_overall_indicators = create_academic_info_table(hs_overall_indicators_data,'TST')
        else:
            hs_overall_indicators = no_data_table('TST')

        if not combined_indicators_data.isnull().all().all():
            combined_indicators = create_academic_info_table(combined_indicators_data,'TST')
        else:
            combined_indicators = no_data_table('TST')

        if not enrollment_indicators_data.isnull().all().all():
            enrollment_indicators = create_academic_info_table(enrollment_indicators_data,'TST')
        else:
            enrollment_indicators = no_data_table('TST')

        if not subgroup_grades_data.isnull().all().all():
            subgroup_grades = create_academic_info_table(subgroup_grades_data,'TST')
        else:
            subgroup_grades = no_data_table('TST')

        if not k8_academic_achievement_data.isnull().all().all():
            k8_academic_achievement = create_academic_info_table(k8_academic_achievement_data,'TST')
        else:
            k8_academic_achievement = no_data_table('TST')

        if not hs_academic_achievement_data.isnull().all().all():
            hs_academic_achievement = create_academic_info_table(hs_academic_achievement_data,'TST')
        else:
            hs_academic_achievement = no_data_table('TST')

        if not k8_academic_progress_data.isnull().all().all():
            k8_academic_progress = create_academic_info_table(k8_academic_progress_data,'TST')
        else:
            k8_academic_progress = no_data_table('TST')

        if not hs_academic_progress_data.isnull().all().all():
            hs_academic_progress = create_academic_info_table(hs_academic_progress_data,'TST')
        else:
            hs_academic_progress = no_data_table('TST')

        if not closing_achievement_gap_data.isnull().all().all():
            closing_achievement_gap = create_academic_info_table(closing_achievement_gap_data,'TST')
        else:
            closing_achievement_gap = no_data_table('TST')

        if not graduation_rate_indicator_data.isnull().all().all():
            graduation_rate_indicator = create_academic_info_table(graduation_rate_indicator_data,'TST')
        ...
        if not strength_of_diploma_indicator_data.isnull().all().all():
            strength_of_diploma_indicator = create_academic_info_table(strength_of_diploma_indicator_data,'TST')
            grad_indicator_container = {'display': 'block'}
        else:
            strength_of_diploma_indicator = {}
            grad_indicator_container = {'display': 'none'}

        if not ela_progress_indicator_data.isnull().all().all():
            ela_progress_indicator = create_academic_info_table(ela_progress_indicator_data,'TST')
        else:
            ela_progress_indicator = no_data_table('TST')

        if not absenteeism_indicator_data.isnull().all().all():
            absenteeism_indicator = create_academic_info_table(absenteeism_indicator_data,'TST')
        else:
