...
def update_about_page(data, school, year):
    ...
    # CHS and CHWM HS)
    if school_index['School Type'].values[0] == 'HS' or school_index['School Type'].values[0] == 'AHS' or school_index['School Type'].values[0] == 'K12' or \
    ...
    if school_index['School Type'].values[0] == 'K8' and len(academic_data_k8.index) == 0:
        ...
        for col in school_test_data.columns:
            school_test_data[col] = pd.to_numeric(school_test_data[col], errors='coerce')

        # Drop 'ELA & Math' Category
        school_test_data = school_test_data.drop(list(school_test_data.filter(regex = 'ELA & Math')), axis = 1)

        # Gets all Grade [x] + ELA columns
#        ela_grade = school_test_data.filter(regex = r'^Grade \d\|ELA|^Grade \d\|Math',axis=1)
            ...
            for s in subject:
                ...
                if total_tested in all_proficiency_data.columns:
                    ...
                    if all_proficiency_data[total_tested].values[0] > 0:
                        ...
                        all_proficiency_data[colz] = all_proficiency_data[colz].divide(all_proficiency_data[total_tested], axis='index')
        ...
        all_proficiency_data.drop(list(all_proficiency_data.filter(regex = 'Total Tested|Total Proficient')), axis = 1, inplace = True)

        # # drop base column (e.g., 'Grade 3|ELA')
        # ela_grade = ela_grade.dropna(axis=1)

        # transpose df
        all_proficiency_data = all_proficiency_data.T.rename_axis('Grade').rename_axis(None, axis=1).reset_index()

        print(all_proficiency_data)
        # split Grade column into two columns and rename what used to be the index
        all_proficiency_data[['Grade','Category']] = all_proficiency_data['Grade'].str.split('|',expand=True)
        all_proficiency_data.rename(columns={0:"Percentage"}, inplace=True)

        colors = plotly.colors.qualitative.Prism

        fig = px.bar(all_proficiency_data, x='Percentage', y='Grade', 
                    color=all_proficiency_data['Category'], barmode='stack',
                    text = [f'{i}%' for i in all_proficiency_data['Percentage']],
                    # text= (ela_grade['Percentage'] * 100).astype(str) + '%',  # Add Percent Sign
                    orientation='h',
    ...
    ## K8 Academic Information
        if school_index['School Type'].values[0] == 'K8' or school_index['School Type'].values[0] == 'K12':
            ...
            if data['4']:
                ...
                final_attendance_data.columns = final_attendance_data.columns.str.replace(r'School$', '', regex=True)
    ...
    if school_index['School Type'].values[0] == 'HS' or school_index['School Type'].values[0] == 'AHS' or school_index['School Type'].values[0] == 'K12' or \
        ...
        if len(academic_data_hs.index) == 0:
            ...
            hs_academic_info.columns = hs_academic_info.columns.str.replace(r'School$', '', regex=True)
