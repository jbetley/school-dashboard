...
def create_proficiency_key() -> list:
    ...
    Returns:
        proficiency_key (list): a list that will be displayed as a dash_table.DataTable
    """
...
def create_iread_ilearn_table(school: str, subject: str, excluded_years: list) -> list:
    """
    Takes school id, subject (ELA/Math), and a list of exlcuded years and returns a
    simple, single header table.
    ...
    if iread_pass_ilearn.empty and iread_nopass_ilearn.empty:
        ...
        iread_ilearn_table = create_simple_table(
...
def create_simple_table(data: pd.DataFrame, label: str) -> list:
    """
...
def create_multi_header_table(data: pd.DataFrame, label: str) -> list:
    """
...
def create_discipline_table(data: pd.DataFrame) -> list:
    ...
    if table_size > 1:
        ...
        nsize_headers = nsize_data.columns.tolist()
        ...
        for header in school_headers:
            tooltip_data[header] = nsize_data[header].astype(str)

        nsize_tooltip = [
            {
                column: {
                    "value": "Total N-Size: " + value
                    if value != "\u2014"
                    else "\u2014",
                    "type": "markdown",
                }
