...
def make_stacked_bar(values: pd.DataFrame, label: str) -> list: #px.bar:
    ...
    # fig['data'][0]['hovertemplate']='Total Tested: %{customdata[1]}<br><br>' + '%{text}: %{customdata[0]}<extra></extra>'
    # fig['data'][1]['hovertemplate']='%{text}: %{customdata[0]}<extra></extra>'
    # fig['data'][2]['hovertemplate']='%{text}: %{customdata[0]}<extra></extra>'
    # fig['data'][3]['hovertemplate']='%{text}: %{customdata[0]}<extra></extra>'

    # TODO: GETTING THERE! Remove Title (replace with total tested) and remove Subject
    ...
    # fig['data'][0]['hovertemplate']='Total Tested: %{customdata[1]}'

    # Add hoverdata
    # TODO: Issue: In a 100% stacked bar chart traces are generated by grouping
    # TODO:a column with values adding up to 100%. In this case, there will always
    # TODO:be 4 values (the number of items in "proficiency_rating"). So each trace
    # TODO:is made up of 4 rows. With a unified hovermode, customdata[0] and customdata[1]
    # TODO:read only the number of rows in the dataframe equal to the number of traces, in
    # TODO:this case 4. So we need to restructure customdata to include: each rating
