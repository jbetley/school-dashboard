...
app = dash.Dash(__name__, server=server, use_pages=True, external_stylesheets=external_stylesheets, suppress_callback_exceptions=True)
...
school_academic_data_k8['Low Grade'] = school_academic_data_k8['Low Grade'].astype(str).str.replace('.0', '', regex=False)
school_academic_data_k8['High Grade'] = school_academic_data_k8['High Grade'].astype(str).str.replace('.0', '', regex=False)
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    dcc.Dropdown(
                                        id='year-dropdown',
                                        options=[{'label':str(y), 'value': str(y)} for y in range(int(current_academic_year) - num_academic_years, int(current_academic_year) + 1)],
                                        style={
                                            'fontSize': '85%',
                                            'fontFamily': 'Roboto, sans-serif',
                                        },
...
### Demographic Data (includes demographics, attendance rates, and state & federal grades)
    ...
    school_demographic_data = all_demographic_data.loc[all_demographic_data['School ID'] == school]
    ...
    if len(school_demographic_data.index) == 0:
        ...
        corp_demographic_data = all_demographic_data.loc[all_demographic_data['School ID'] == school_info['GEO Corp'].values[0]]

        # get school demographic data for selected year and save to dict
        school_demographic_selected_year = school_demographic_data.loc[school_demographic_data['Year'] == demographic_year]
        school_demographic_selected_year_dict = school_demographic_selected_year.to_dict()

        # get corp demographic data for selected year and save to dict
        corp_demographic_selected_year = corp_demographic_data.loc[corp_demographic_data['Year'] == demographic_year]
        ...
        if excluded_years:
            corp_demographic_data = corp_demographic_data[~corp_demographic_data['Year'].isin(excluded_years)]
        ...
        # transpose for display
        school_attendance_rate = school_attendance_rate.set_index('Year').T.rename_axis(None, axis=1).reset_index()
        school_attendance_rate.drop('index', inplace=True, axis=1)

        corp_attendance_rate = corp_demographic_data[['Year','Avg Attendance']]
        corp_attendance_rate = corp_attendance_rate.set_index('Year').T.rename_axis(None, axis=1).reset_index()
        ...
        for col in corp_attendance_rate.columns:
            corp_attendance_rate[col] = pd.to_numeric(corp_attendance_rate[col], errors='coerce')

        for col in school_attendance_rate.columns:
            school_attendance_rate[col] = pd.to_numeric(school_attendance_rate[col], errors='coerce')

        # add accountability category
        school_attendance_rate.replace(0, np.nan, inplace=True)
        school_attendance_rate['Category'] = '1.1.a. Attendance Rate (compared to school corporation average)'
        last_col = school_attendance_rate.pop('Category')
        school_attendance_rate.insert(0,'Category', last_col)
        corp_attendance_rate['Category'] = '1.1.a. Attendance Rate (compared to school corporation average)'
        ...
        merged_data = school_attendance_rate.merge(corp_attendance_rate, on ='Category', how='left')
        ...
        result_attendance_rate = pd.DataFrame(school_attendance_rate.values - corp_attendance_rate.values)
        ...
        attendance_data = merged_data.merge(result_attendance_rate, on ='Category', how='left')
        ...
        attendance_limits = [0,-.01,-.01]   # threshold limits for rating calculations
...
## Financial, Organizational, Average Daily Membership, and Federal Audit Findings
    ...
    if school_adm.sum(axis = 1).values[0] == 0:   # this is true if all columns are equal to 0
        school_adm_dict = {}
        ...
        if operating_years_by_adm > max_display_years:
            school_adm.drop(columns=school_adm.columns[:(operating_years_by_adm - max_display_years)], axis=1, inplace=True)
...
### Academic Data
    ...
    if school_info['School Type'].values[0] == 'K8' or school_info['School Type'].values[0] == 'K12':
        ...
        filtered_school_academic_data_k8 = school_academic_data_k8[~school_academic_data_k8['Year'].isin(excluded_years)]
        k8_school_data =  filtered_school_academic_data_k8.loc[filtered_school_academic_data_k8['School ID'] == school]
        ...
        if (len(k8_school_data.index) == 0):
            ...
            # school corporation.
            k8_corp_rates_filtered = corporation_rates[~corporation_rates['Year'].isin(excluded_years)]
            k8_corp_rate_filtered = k8_corp_rates_filtered.loc[(k8_corp_rates_filtered['Corp ID'] == school_info['GEO Corp'].values[0])]
            ...
            k8_corp_rate_filtered = k8_corp_rate_filtered.filter(regex = r'Total Tested$|Total Proficient$|IREAD Pass N|IREAD Test N|Year',axis=1)
            ...
            k8_corp_rate_filtered = k8_corp_rate_filtered[k8_corp_rate_filtered.columns[valid_mask]]
            ...
            for col in k8_corp_rate_filtered.columns:
                k8_corp_rate_filtered[col] = pd.to_numeric(k8_corp_rate_filtered[col], errors='coerce')
            ...
            for s in subject:
                for c in categories:
                    ...
                    if proficient not in missing_cols:
                        ...
                        k8_corp_rate_data[new_col] = k8_corp_rate_data[proficient] / k8_corp_rate_data[tested]
            ...
            adjusted_corp_total_math_proficient = k8_corp_rate_data.filter(regex=r"Grade.+?Math Total Proficient")
            adjusted_corp_total_math_tested = k8_corp_rate_data.filter(regex=r"Grade.+?Math Total Tested")
            k8_corp_rate_data['Total|Math Proficient %'] = adjusted_corp_total_math_proficient.sum(axis=1) / adjusted_corp_total_math_tested.sum(axis=1)

            adjusted_corp_total_ela_proficient = k8_corp_rate_data.filter(regex=r"Grade.+?ELA Total Proficient")
            adjusted_corp_total_ela_tested = k8_corp_rate_data.filter(regex=r"Grade.+?ELA Total Tested")
            k8_corp_rate_data['Total|ELA Proficient %'] = adjusted_corp_total_ela_proficient.sum(axis=1) / adjusted_corp_total_ela_tested.sum(axis=1)

            # calculate IREAD Pass %
            if 'IREAD Pass N' in k8_school_data:
                k8_corp_rate_data['IREAD Pass %'] = k8_corp_rate_data['IREAD Pass N'] / k8_corp_rate_data['IREAD Test N']
            ...
            k8_corp_rate_data = k8_corp_rate_data.filter(regex = r'\|ELA Proficient %$|\|Math Proficient %$|^IREAD Pass %|^Year$',axis=1)
            ...
            k8_corp_rate_data = k8_corp_rate_data.reset_index()   # no drop because index was previous set to year
            ...
            for c in k8_school_data.columns:
                ...
                k8_result[c +'+/-'] = calculate_difference(k8_school_data[c + 'School'],k8_corp_data[c + 'Corp Rate'])
...
#### Academic Metrics (k8)
            ...
            for y in range(0,(len(k8_school_metric_data.columns)-1)):
                year_over_year_values_values[k8_school_metric_data.columns[y]] = calculate_year_over_year(k8_school_metric_data.iloc[:,y],k8_school_metric_data.iloc[:,y+1])
            ...
            year_over_year_values.drop([col for col in year_over_year_values.columns if 'Corp Rate' in col or '+/-' in col],axis=1,inplace=True)

            # TODO: See above - is this redundant?
            # clean up df for display
            year_over_year_values_values = year_over_year_values_values.set_index(['Category']).add_suffix('+/-').reset_index()
            year_over_year_values_values['Category'] = year_over_year_values_values['Category'].str.replace('Proficient %', '').str.strip()
            ...
            merged_years_cols = [val for pair in zip(school_years_cols, year_over_year_values_values_cols) for val in pair]
            ...
            [year_over_year_values.insert(i,str(year_over_year_values.columns[i-1])[:7 - 3] + 'Rating' + str(i), year_over_year_values.apply(lambda x : getRating(x[year_over_year_values.columns[i-1]], years_limits,1), axis = 1)) for i in range(year_over_year_values.shape[1], 1, -2)]
            ...
            rating_cols = list(col for col in year_over_year_values.columns if 'Rating' in col)
            ...
            if col_pair:
                for k, v in col_pair:
                    year_over_year_values[v] = np.where(year_over_year_values[k] == 0, 'DNMS', year_over_year_values[v])
...
## TODO: THIS CONDITIONAL IS BROKEN
    ...
    if school_info['School Type'].values[0] == 'HS' or school_info['School Type'].values[0] == 'K12' or school_info['School Type'].values[0] == 'AHS' or \
        (school_info['School ID'].values[0] == '5874' and int(year) < 2021):

        # If the school is a HS or AHS -> all k8 dicts/json will be empty
        if school_info['School Type'].values[0] == 'HS' or school_info['School Type'].values[0] == 'AHS':
            k8_academic_data_json = {}
            diff_to_corp_json = {}
            year_over_year_values_json = {}
            iread_data_json = {}
        ...
        if school_info['School Type'].values[0] == 'HS' or school_info['School Type'].values[0] == 'K12' or \
            (school_info['School ID'].values[0] == '5874' and int(year) < 2021):
                ahs_metrics_data_json = {}
...
### TODO:   1) ADD SAT GRADE 11/ACT SCORES (DATASET IS MISSING 2019-2021?)
            ...
            hs_corp_data = filtered_academic_data_hs.loc[(filtered_academic_data_hs['Corp ID'] == school_info['GEO Corp'].values[0])]
            ...
            hs_corp_rate_data = hs_corp_rate_data.loc[(hs_corp_rate_data['Year'].isin(hs_corp_data['Year']))]
            ...
            missing_year = list(sorted(set(hs_corp_data['Year'].tolist()) - set(hs_corp_rate_data['Year'].tolist())))
            ...
            if missing_year:
                for y in missing_year:
                    hs_corp_rate_data = pd.concat([hs_corp_rate_data,pd.DataFrame(np.nan, columns=hs_corp_rate_data.columns, index=range(1))], ignore_index=True)
            ...
            if 'Grade 10' not in '\t'.join(missing_cols):
                for e in eca_categories:
                    ...
                    hs_corp_data[new_col] = hs_corp_rate_data[passN] / hs_corp_rate_data[testN]
...
###       Need to revisit this entire section
        ...
        ## State Avg Graduation Rate
            ...
            filtered_academic_data_hs['Total|Graduates'] = pd.to_numeric(filtered_academic_data_hs['Total|Graduates'], errors='coerce')
            filtered_academic_data_hs['Total|Cohort Count'] = pd.to_numeric(filtered_academic_data_hs['Total|Cohort Count'], errors='coerce')

            # NOTE: exclude AHS from graduation rate calculation due to the inapplicability of grad rates to the model
            filtered_academic_data_hs['Total|Graduates'] = filtered_academic_data_hs.loc[filtered_academic_data_hs['School Type'] != 'AHS', 'Total|Graduates']
            filtered_academic_data_hs['Total|Cohort Count'] = filtered_academic_data_hs.loc[filtered_academic_data_hs['School Type'] != 'AHS', 'Total|Cohort Count']

            state_grad_average = filtered_academic_data_hs.groupby('Year', as_index=False).sum().eval('State_Grad_Average = `Total|Graduates` / `Total|Cohort Count`')
...
##### AHS/HS Academic Metrics
            ...
            if school_info['School Type'].values[0] == 'AHS':
                ...
                [ahs_state_grades.insert(i,str(ahs_metric_cols[i-2]) + 'Rating' + str(i), ahs_state_grades.apply(lambda x : getRating(x[ahs_state_grades.columns[i-1]], letter_grade_limits,4), axis = 1)) for i in range(ahs_state_grades.shape[1], 1, -1)]
