...
#    color=['#98abc5','#8a89a6','#7b6888','#6b486b','#a05d56','#d0743c','#ff8c00']
    ...
    # Add values to this list to display more data from index
...
#                        style_as_list_view=True
    ...
    if (school_index['School Type'].values[0] == 'K8' and not data['3']) or \
        school_index['School Type'].values[0] == 'K12' and (not data['3'] and not data['4']) or \
        (school_index['School Type'].values[0] == 'AHS' or school_index['School Type'].values[0] == 'HS') and not data['4']:

        hold_harmless_string = ''
        letter_grade_table = empty_table
        ...
        elif (school_index['School Type'].values[0] == 'K12'):
            
            # k8_letter_grades_json
            json_data = json.loads(data['3'])


#### TODO: Display both K8 and 9-12 Grades if they exist
            # hs_letter_grades_json
            # json2_data = json.loads(data['4'])
            # hs_letter_grades = pd.DataFrame.from_dict(json2_data)

        elif (school_index['School Type'].values[0] == 'AHS' or school_index['School Type'].values[0] == 'HS'):
            
            # hs_letter_grades_json            
            json_data = json.loads(data['4'])

        grade_data = pd.DataFrame.from_dict(json_data)

        # 2019 and 2020 were 'Hold Harmless' years for all schools
        if '2019' in grade_data:
        ...
        letter_grade_table = [
                    dash_table.DataTable(
                        ...
                        columns = [{'name': str(i), 'id': str(i)} for i in grade_data.columns],
                        style_data={
                            'fontSize': '12px',
                            'fontFamily': 'Roboto, sans-serif',
                            'border': 'none'
...
#                            'height': 'auto',
    ...
    # school_adm_dict        
    school_adm = pd.DataFrame.from_dict(data['5'])
    ...
    if len(school_adm.index) == 0:
        ...
        adm_fig.update_yaxes(title='', showgrid=True, gridcolor='#b0c4de') # ticks='outside', tickcolor='#a9a9a9', 
    ...
    ethnicity_school = school_demographics.loc[:, (school_demographics.columns.isin(ethnicity)) | (school_demographics.columns.isin(['Corporation Name','Total Enrollment']))]
    ethnicity_corp = corp_demographics.loc[:, (corp_demographics.columns.isin(ethnicity)) | (corp_demographics.columns.isin(['Corporation Name','Total Enrollment']))]
    ...
    if not ethnicity_school.empty:
        ...
        ethnicity_anno_txt = ', '.join(no_show.index.values.astype(str)) # create string of ethnicities with fewer than 1/2% of representation
        ...
        # force wrap (use categories for no wrap)
    ...
    return school_name, info_table, letter_grade_table, hold_harmless_string, enroll_title, enroll_table, adm_fig, ethnicity_title, ethnicity_fig, subgroup_title, status_fig
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    dcc.Graph(id='adm_fig', figure={},config={'displayModeBar': False})
                                ],
                                className = 'pretty_container six columns'
                            ),
                        ],
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    dcc.Graph(id='status-fig', figure={},config={'displayModeBar': False})
                                ],
                                className = 'pretty_container six columns'
                            ),
                            html.Div(
                                [
                                    ...
                                    dcc.Graph(id='ethnicity-fig', figure={},config={'displayModeBar': False})
                                ],
                                className = 'pretty_container six columns'
                            ),
                        ],
